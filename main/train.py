...
stft = ConvSTFT(win_len, win_inc, fft_len=fft_len, win_type=win_type, feature_type='real', fix=True)
istft = ConviSTFT(win_len, win_inc, fft_len=fft_len, win_type=win_type, feature_type='real', fix=True)
...
class Separation(sb.Brain):
    def compute_forward(self, mix, targets, stage, noise=None):
        ...
        est_source = torch.cat(
            [
                ...
                # istft(sep_h[i], phase).unsqueeze(-1)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            plt.plot(valid_losses, ls='--', label='validation loss')
            plt.xlabel('Epoch')
