...
def init_kernels(win_len, win_inc, fft_len, win_type=None, invers=False):
    ...
    kernel = kernel[:, None, :]
    return torch.from_numpy(kernel.astype(np.float32)).to(device), torch.from_numpy(window[None,:,None].astype(np.float32)).to(device)
...
class ConviSTFT(nn.Module):
    ...
    def __init__(self, win_len, win_inc, fft_len=None, win_type='hamming', feature_type='real', fix=True):
        ...
        self.stride = win_inc
        self.dim = self.fft_len
        # self.window = nn.Parameter(window, requires_grad=(not fix))
        # self.enframe = nn.Parameter(torch.eye(win_len)[:,None,:].to(device), requires_grad=(not fix))
        self.register_buffer('window', window)
        self.register_buffer('enframe', torch.eye(win_len)[:,None,:].to(device))
    ...
    def forward(self, inputs, phase=None):
        ...
        if torch.squeeze(outputs).dim() == 1:
