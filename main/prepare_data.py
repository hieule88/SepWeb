...
def get_wsj_files(data_type, output_dir, save_fs="wav16k", min_maxs=["max"]):
    ...
    for min_max in min_maxs:
        ...
        TaskFile = os.path.join(
            filedir, "SpeechSeparation", "dataset", "mix_2_spk_" + data_type + ".txt"
        )
        ...
        for i, line in tqdm(enumerate(C), total=num_files):
...
if __name__ == "__main__":
    ...
    root = root.split('/')
    root = '/'.join(root[:-2])
    data_path = os.path.join(root, "SpeechSeparation" ,"dataset")
    # data_type = ['vd', 'tr']
    data_type = ['tr']
    for i in data_type:
